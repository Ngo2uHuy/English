...
const allTopicIds = GRAMMAR_TOPICS.map(t => t.id);
const roadmapTopicIds = EXPERT_ROADMAP.flatMap(s => s.topics);

console.log('Total topics in GRAMMAR_TOPICS:', allTopicIds.length);
console.log('Total topics in EXPERT_ROADMAP:', roadmapTopicIds.length);

const missing = allTopicIds.filter(id => !roadmapTopicIds.includes(id));
console.log('Missing topics:', missing);

const invalid = roadmapTopicIds.filter(id => !allTopicIds.includes(id));
console.log('Invalid topic IDs in roadmap:', invalid);
